...
Packaging: this release also fixes two problems with the bundled Podman engine: jobs failing before their first step because it could not open the tunnel device that gives each job container a network, and job images containing files not owned by root — most Ubuntu-based images — failing to unpack.
...
Empaquetado: este lanzamiento corrige además dos problemas del motor Podman incluido: los trabajos fallaban antes de su primer paso porque no podía abrir el dispositivo de túnel que da red a cada contenedor de trabajo, y las imágenes de trabajo con archivos que no pertenecen a root —la mayoría de las basadas en Ubuntu— no se descomprimían.
